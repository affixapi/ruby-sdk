  ...
  class ClientResponse
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::ClientResponse` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::ClientResponse`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'client_secret')
        if (value = attributes[:'client_secret']).is_a?(Array)
      ...
      if attributes.key?(:'webhook_uri')
        self.webhook_uri = attributes[:'webhook_uri']
      end
    end
    ...
    def hash
      [client_id, client_secret, redirect_uris, name, webhook_uri].hash
    end
