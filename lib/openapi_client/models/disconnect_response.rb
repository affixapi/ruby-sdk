  ...
  class DisconnectResponse
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::DisconnectResponse` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::DisconnectResponse`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'disconnected')
        self.disconnected = attributes[:'disconnected']
      end
    end
    ...
    def valid?
      ...
      disconnected_validator = EnumAttributeValidator.new('Boolean', ["true"])
      return false unless disconnected_validator.valid?(@disconnected)
      true
    end

    # Custom attribute writer method checking allowed values (enum).
    # @param [Object] disconnected Object to be assigned
    def disconnected=(disconnected)
      validator = EnumAttributeValidator.new('Boolean', ["true"])
      unless validator.valid?(disconnected)
        fail ArgumentError, "invalid value for \"disconnected\", must be one of #{validator.allowable_values}."
      end
      @disconnected = disconnected
    end
