  ...
  class InlineResponse401
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::InlineResponse401` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::InlineResponse401`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def error=(error)
      validator = EnumAttributeValidator.new('String', ["authentication_error"])
    ...
    def code=(code)
      validator = EnumAttributeValidator.new('String', ["incorrect_username_or_password", "incorrect_mfa_or_otp", "reauthentication_required // cookies are now expired", "additional_account_setup_required // password reset, etc", "captcha_challenge // amazon type1 captcha challenge (currently unsovable)", "no_valid_accounts // not used currently but reserved for future use", "insufficient_permission // not enough privileges (ie not an admin, or not enough provider scopes on a provider api token for the Affix scopes requested)"])
